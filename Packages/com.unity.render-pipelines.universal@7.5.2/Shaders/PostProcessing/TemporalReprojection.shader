...
Shader "Hidden/Universal Render Pipeline/TemporalReprojection"
{
	...
	#include "IncDepth.hlsl"
	#include "IncNoise.hlsl"
	...
	uniform sampler2D _ReprojectionBuffer_00;
	uniform sampler2D _ReprojectionBuffer_01;
	...
	#else
		OUT.cs_pos = TransformObjectToHClip(IN.vertex.xyz);
	#endif
	...
	float4 temporal_reprojection(float2 ss_txc, float2 ss_vel, float vs_dist)
	{
		...
		float4 texel0 = sample_color(_CurrentTex, ss_txc - _JitterUV.xy);
	#else
		float4 texel0 = sample_color(_CurrentTex, ss_txc);
	#endif

		if (_FirstTimeExcute == 1) 
		{
			return texel0;
		}
		...
		float2 texel_vel = ss_vel / _CurrentTex_TexelSize.xy;
		...
		float2 ss_offset01 = k_min_max_support * float2(-_CurrentTex_TexelSize.x, _CurrentTex_TexelSize.y);
		float2 ss_offset11 = k_min_max_support * float2(_CurrentTex_TexelSize.x, _CurrentTex_TexelSize.y);
		float4 c00 = sample_color(_CurrentTex, uv - ss_offset11);
		float4 c10 = sample_color(_CurrentTex, uv - ss_offset01);
		float4 c01 = sample_color(_CurrentTex, uv + ss_offset01);
		float4 c11 = sample_color(_CurrentTex, uv + ss_offset11);
	...
	f2rt frag(v2f IN)
	{
		...
		if (_FirstTimeExcute == 1) 
		{
			OUT.buffer = color_temporal;
			OUT.screen = color_temporal;
			return OUT;
		}
		...
		float vel_mag = length(ss_vel * _CurrentTex_TexelSize.zw);
		...
		#if UNJITTER_COLORSAMPLES
			float4 color_motion = sample_color_motion(_CurrentTex, IN.ss_txc - _JitterUV.xy, ss_vel);
		#else
			float4 color_motion = sample_color_motion(_CurrentTex, IN.ss_txc, ss_vel);
		#endif
