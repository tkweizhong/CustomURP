...
{
	return depth_resolve_linear(SampleSceneDepth(uv).x);
}
...
float3 find_closest_fragment_3x3(float2 uv)
{
	...
	float3 dtl = float3(-1, -1, SampleSceneDepth(uv - dv - du).x);
	float3 dtc = float3( 0, -1, SampleSceneDepth(uv - dv).x);
	float3 dtr = float3( 1, -1, SampleSceneDepth(uv - dv + du).x);

	float3 dml = float3(-1, 0, SampleSceneDepth(uv - du).x);
	float3 dmc = float3( 0, 0, SampleSceneDepth(uv).x);
	float3 dmr = float3( 1, 0, SampleSceneDepth(uv + du).x);

	float3 dbl = float3(-1, 1, SampleSceneDepth(uv + dv - du).x);
	float3 dbc = float3( 0, 1, SampleSceneDepth(uv + dv).x);
	float3 dbr = float3( 1, 1, SampleSceneDepth(uv + dv + du).x);
...
float2 find_closest_fragment_3x3_packed(in float2 uv)
{
	...
	float dtl = trunc(s * SampleSceneDepth(uv - dv - du).x) + 0.1010;// -+-+
	float dtc = trunc(s * SampleSceneDepth(uv - dv).x)      + 0.0010;
	float dtr = trunc(s * SampleSceneDepth(uv - dv + du).x) + 0.0110;
	float dml = trunc(s * SampleSceneDepth(uv - du).x)      + 0.1000;
	float dmc = trunc(s * SampleSceneDepth(uv).x)           + 0.0000;
	float dmr = trunc(s * SampleSceneDepth(uv + du).x)      + 0.0100;
	float dbl = trunc(s * SampleSceneDepth(uv + dv - du).x) + 0.1001;
	float dbc = trunc(s * SampleSceneDepth(uv + dv).x)      + 0.0001;
	float dbr = trunc(s * SampleSceneDepth(uv + dv + du).x) + 0.0101;
...
float3 find_closest_fragment_5tap(float2 uv)
{
	...
	float dtl = SampleSceneDepth(uv + tl).x;
	float dtr = SampleSceneDepth(uv + tr).x;
	float dmc = SampleSceneDepth(uv).x;
	float dbl = SampleSceneDepth(uv + bl).x;
	float dbr = SampleSceneDepth(uv + br).x;
