...
namespace UnityEngine.Rendering.Universal.Internal
{
    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        public void Setup(in RenderTextureDescriptor baseDescriptor, in RenderTargetHandle source, in RenderTargetHandle destination, in RenderTargetHandle depth, 
            in RenderTargetHandle internalLut, bool enableSRGBConversion)
        {
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            bool isFXAA = cameraData.antialiasing == AntialiasingMode.FastApproximateAntialiasing;
            //FXAA pass is the last pass;
            ...
            int GetDestination()
            {
                ...
                //Note : we need create another "RenderTarget" when FXAA enabled.
                else if (destination == m_Source.id && 
                    (m_Descriptor.msaaSamples > 1 || isFXAA))
                {
            ...
            else if (cameraData.antialiasing == AntialiasingMode.TemporalAntialiasing)
            {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
                ...
                //we rendering  to "temp target" if FXAA enabled, otherwise rendering to "camera target".
                if (isFXAA)
                    cameraTarget = GetDestination();
                else
                ...
                bool finishPostProcessOnScreen = cameraData.resolveFinalTarget ||
                    (m_Destination == RenderTargetHandle.CameraTarget /*|| m_HasFinalPass == true*/);
                ...
                else
                {
                    ...
                    if (isFXAA)
                    {
                        ...
                        {
                            Swap();
                            DoFastApproximateAntialiasing(ref cameraData, cmd, GetSource(), m_Source.id);
                        }
                    }
                    // TODO: We need a proper camera texture swap chain in URP.
                    // For now, when render post-processing in the middle of the camera stack (not resolving to screen)
        ...
        void DoFastApproximateAntialiasing(ref CameraData cameraData, CommandBuffer cmd,
            int source, int destination)
        {
            ...
            //RenderTargetIdentifier desRenderTargetIdentifier = new RenderTargetIdentifier(destination);
